    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <View style={s.headerBadge}>
          ...
          <Text style={s.headerBadgeText}>Aldeota</Text>
        </View>
      </View>
      ...
      <View style={s.mapCard}>
        ...
        <View style={s.mapOverlay}>
          <View style={s.mapYouPin}>
            <View style={s.mapYouDot} />
            <Text style={s.mapYouLabel}>Você</Text>
          </View>
        </View>
        <View style={s.mapBadgeRow}>
          {Object.entries(RISK_CONFIG).map(([key, val]) => (
            <View key={key} style={[s.mapBadge, { backgroundColor: val.bg, borderColor: val.border }]}>
              <View style={[s.mapBadgeDot, { backgroundColor: val.iconColor }]} />
              <Text style={[s.mapBadgeText, { color: val.text }]}>{val.label}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {activeTab === 'bairros' && (
          <View style={s.grid}>
            {bairrosFiltrados.map(b => {
              const rc = RISK_CONFIG[b.risk as keyof typeof RISK_CONFIG];
              const isSel = selectedB === b.name;
              return (
                <TouchableOpacity
                  key={b.name}
                  style={[s.bCard, { borderColor: isSel ? rc.iconColor : rc.border }, isSel && { borderWidth: 2 }]}
                  onPress={() => setSelectedB(isSel ? null : b.name)}
                  activeOpacity={0.75}
                >
                  <View style={s.bCardTop}>
                    <View style={[s.bRiskDot, { backgroundColor: rc.iconColor }]} />
                    <Text style={s.bName}>{b.name}</Text>
                    {b.incidents > 0 && (
                      <View style={[s.bBadge, { backgroundColor: rc.iconColor }]}>
                        <Text style={s.bBadgeText}>{b.incidents}</Text>
                      </View>
                    )}
                  </View>
                  <View style={[s.bRiskTag, { backgroundColor: rc.bg }]}>
                    <Ionicons name={rc.icon} size={11} color={rc.iconColor} />
                    <Text style={[s.bRiskLabel, { color: rc.text }]}>{rc.label}</Text>
                  </View>
                  {isSel && (
                    <Text style={[s.bDetail, { color: rc.text }]}>
                      {b.incidents === 0
          ...
          <View style={s.oList}>
            {ocorrenciasFiltradas.length === 0 && (
              <View style={s.emptyState}>
                ...
                <Text style={s.emptyText}>Nenhuma ocorrência encontrada</Text>
              </View>
            )}
            {ocorrenciasFiltradas.map(o => {
              const rc = RISK_CONFIG[o.risco as keyof typeof RISK_CONFIG];
              const ti = TIPO_ICON[o.tipo] || { icon: 'alert-outline' as const, color: Colors.pinkBtn };
              return (
                <View key={o.id} style={s.oCard}>
                  <View style={[s.oIconBox, { backgroundColor: ti.color + '18' }]}>
                    <Ionicons name={ti.icon} size={22} color={ti.color} />
                  </View>
                  <View style={s.oInfo}>
                    <Text style={s.oTipo}>{o.tipo}</Text>
                    <View style={s.oMetaRow}>
                      <Ionicons name="location-outline" size={11} color="#999" />
                      <Text style={s.oMeta}>{o.bairro}</Text>
                      <Ionicons name="time-outline" size={11} color="#999" style={{ marginLeft: 8 }} />
                      <Text style={s.oMeta}>{o.hora}</Text>
                    </View>
                  </View>
                  <View style={[s.oRiskTag, { backgroundColor: rc.bg, borderColor: rc.border }]}>
                    <Text style={[s.oRiskText, { color: rc.text }]}>{rc.label}</Text>
                  </View>
                </View>
              );
            })}
        ...
        <View style={s.yourLocation}>
          <View style={s.yourLocationLeft}>
            <Ionicons name="navigate" size={16} color={Colors.pinkBtn} />
            <Text style={s.yourLocationText}>
              Você está em <Text style={s.yourLocationBold}>Aldeota</Text> — zona segura
            </Text>
          </View>
          <View style={[s.locationDot, { backgroundColor: '#43A047' }]} />
        </View>

        <TouchableOpacity style={s.reportBtn} onPress={() => router.push('/home')} activeOpacity={0.85}>
          <View style={s.reportBtnInner}>
            <Ionicons name="add-circle" size={20} color="#fff" />
            <Text style={s.reportBtnText}>Reportar ocorrência neste local</Text>
          </View>
        </TouchableOpacity>

        <Text style={s.updateText}>
          <Ionicons name="refresh" size={11} color="#bbb" /> Atualiza a cada 15 minutos
        </Text>
